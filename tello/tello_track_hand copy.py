    ...
    if results.multi_hand_landmarks:
      for hand_no,hand_landmarks in enumerate(results.multi_hand_landmarks):
        mp_drawing.draw_landmarks(
            ...
            mp_drawing_styles.get_default_hand_connections_style())
        #global lr,fb,ud,y,a
        if hand_landmarks.landmark[mp_hands.HandLandmark(0).value].x * image_width < (image_width*1/3):
            #print('r')
            lr = -20
        elif hand_landmarks.landmark[mp_hands.HandLandmark(0).value].x * image_width < (image_width*2/3):
            #print('m')
            lr = 0
        elif hand_landmarks.landmark[mp_hands.HandLandmark(0).value].x * image_width < (image_width):
            #print('l')
            lr = 20
        if hand_landmarks.landmark[mp_hands.HandLandmark(0).value].y * image_height < (image_height*1/3):
            #print('t')
            ud = 20
        elif hand_landmarks.landmark[mp_hands.HandLandmark(0).value].y * image_height < (image_height*2/3):
            #print('m')
            ud = 0
        elif hand_landmarks.landmark[mp_hands.HandLandmark(0).value].y * image_height < (image_height):
            #print('b')
            ud = -20
      # else:
      #   lr = 0
